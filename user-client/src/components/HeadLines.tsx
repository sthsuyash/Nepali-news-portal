        ...
        <div className='bg-white shadow flex flex-wrap'>
            ...
            <div className='flex md:w-[calc(100%-170px)] w-full'>
                <div className='flex w-full justify-start items-center'>
                    <Marquee>
                        {Object.entries(news).map(([category, articles], i) => (
                            // Type assertion to specify articles as an array of objects
                            Array.isArray(articles) && articles.length > 0 && articles.map((article, j) => (
                                <Link
                                    key={`${category}-${i}-${j}`}
