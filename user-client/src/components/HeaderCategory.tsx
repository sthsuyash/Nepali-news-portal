        ...
        <div className='w-full'>
            <div className='bg-[#c80000] w-full text-white uppercase font-semibold relative'>
                <div className='px-8 flex justify-between items-center relative h-[48px]'>
                    <div onClick={() => setCateShow(!cateShow)} className={`text-3xl flex lg:hidden font-bold h-full w-[48px] cursor-pointer justify-center items-center ${cateShow ? 'bg-[#00000026]' : ''} hover:bg-[#00000026]`}>
                        <BsList />
                    </div>
                    <div className='flex-wrap hidden lg:flex'>
                        <Link className={`px-6 font-medium py-[13px] ${path === '/' ? 'bg-[#00000026]' : ''}`} href={'/'} >मुख्य पृष्ठ</Link>
                        {
                            categories.length > 0 && categories.map((c, i) => (
                                // @ts-expect-error
                                <Link key={i} className={`px-6 font-medium py-[13px] ${path === `/news/category/${c.category}` ? 'bg-[#00000026]' : ''}`} href={`/news/category/${c.category}`} >{c.category}</Link>
                            ))
                        }
                    </div>
            ...
            {
                cateShow && <div className='flex flex-wrap lg:hidden py-2 px-[30px]'>
                    <Link className={`px-4 font-medium py-[5px] ${path === '/' ? 'bg-[#00000026]' : ''}`} href={'/'} >मुख्य पृष्ठ</Link>
                    {
                        categories.map((c, i) => (
                            // @ts-expect-error
                            <Link key={i} className={`px-4 font-medium py-[5px] ${path === `/news/category/${c.category}` ? 'bg-[#00000026]' : ''}`} href={`/news/category/${c.category}`} >{c.category}</Link>
                        ))
                    }
                </div>
            }
